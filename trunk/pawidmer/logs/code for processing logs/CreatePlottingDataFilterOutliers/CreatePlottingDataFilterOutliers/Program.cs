...
namespace CreatePlottingData
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            int ignoreEnd = 2000;
            int ignoreBeginning = 2000;
            string saveTo = "ResponseDBType1.txt";
            List<string> cons = new List<string> { "Cons1", "Cons2", "Cons4", "Cons6", "Cons8", "Cons10", "Cons12", "Cons16" };
            string afterconsCount = "_Thread";
            int filterMin = 0; int filterMax = 10;
                ...
                for (int i = 0; i < cons.Count; ++i)
                {
                    ...
                    foreach (String filename2 in dlg.FileNames)
                    {
                        if (filename2.Contains(cons[i]))
                        {
                            ...
                            lines.AddRange(tempLines.GetRange(ignoreBeginning, tempLines.Count - (ignoreEnd + ignoreBeginning)));
                        }
                    }
                    ...
                    for (int j = 0; j < lines.Count(); ++j)
                    {
                        ...
                        dtemp = temp / 1000000;
                        if (dtemp >= filterMin && dtemp <= filterMax)
                            total += dtemp;
                    }
