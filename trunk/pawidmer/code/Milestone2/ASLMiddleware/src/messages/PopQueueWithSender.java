...
public class PopQueueWithSender extends Request {
	
	public int senderID = -1;
	
	public PopQueueWithSender(int aQueueID,int aAssociatedClientID, int aSenderID) {
		queueID = aQueueID;
		messageType = POP_QUEUE_WITH_SENDER;
		associatedClientID = aAssociatedClientID;
		senderID = aSenderID;
	}

	public PopQueueWithSender(DataInputStream input) throws IOException {
		messageType = POP_QUEUE_WITH_SENDER;
	...
	public Response executeQuery(Connection connection) {
		try {
			String sqlString = "select * from pop_queue_with_sender(?,?,?)";
