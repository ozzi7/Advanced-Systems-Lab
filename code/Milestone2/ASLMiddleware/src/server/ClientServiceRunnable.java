...
public class ClientServiceRunnable implements Runnable {
	...
	private EventLogger serviceTimeLogger;
	...
	public ClientServiceRunnable(Socket aClientServiceSocket,
		...
		try {
			...
			dataInFromClient = new DataInputStream(new BufferedInputStream(inFromClient));
			Thread.sleep(100);
			output = new DataOutputStream(new BufferedOutputStream(clientServiceSocket.getOutputStream()));
	...
	@Override
	public void run() {
		long startTimeService = System.nanoTime();
		while (!Thread.currentThread().isInterrupted()) {
			try {
				...
				serviceTimeLogger.logNoTimestamp((System.nanoTime()/1000000000) + " " +String.valueOf(System.nanoTime()-startTimeService));
				...
				errorLogger.log("Interrupted Exception");
				return;
			}
		}
		try {
	...
	private void initLogger() {
		try {
			...
			serviceTimeLogger= new EventLogger("ClientServiceThreads","ServiceTime_"+dataGen.getRandomInt(1, 1000000));
			serviceTimeLogger.logging = Settings.SERVICE_TIME_LOGGING;
		} catch (IOException e1) {
